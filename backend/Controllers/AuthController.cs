...
public class AuthController : ControllerBase
{
    ...
    // ── PUT /api/auth/profile/update ────────────────────────── [SCRUM-88 & SCRUM-91]
    [HttpPut("profile/update")]
    [Authorize]
    public async Task<IActionResult> UpdateProfile([FromBody] UpdateProfileNameDto dto)
    {
        try
        {
            // SCRUM-91: Secure restriction matching authenticated claim context identity strictly
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (string.IsNullOrEmpty(userIdClaim))
                return Unauthorized(new ResponseDto { Success = false, Message = "Invalid or expired session token authority context." });

            int userId = int.Parse(userIdClaim);
            var user = await _db.Users.FindAsync(userId);

            if (user == null)
                return NotFound(new ResponseDto { Success = false, Message = "User matching account identifier database record registry not found." });
            ...
            return Ok(new ResponseDto { 
                Success = true, 
                Message = "Profile updated securely details", 
